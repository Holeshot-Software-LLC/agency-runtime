...
test("app.js roster and empty evidence renderers expose actionable, scoped controls", () => {
  const harness = createAppHarness(() => {
    throw new Error("render-only test does not fetch");
  });
  harness.api.state.roster = [
    { agent_slug: "security-reviewer", capabilities: ["security"], division: "review" },
    { agent_slug: "generalist", capabilities: [] },
  ];
  ...
  assert.equal(harness.node("roster-count").textContent, "2");
  ...
  assert.equal(snapshotNodes.filter((node) => node.type === "button").length, 2);
...
test("app.js honors reduced motion and keeps live toggle semantics synchronized", () => {
  const harness = createAppHarness(() => {
    throw new Error("this test does not fetch");
  });
...
test("app.js cancels stale full refresh generations before they can render", async () => {
  ...
  assert.deepEqual(
    ...
    ["/api/live?limit=100", "/api/hosts", "/api/roster", "/api/snapshots", "/api/config"],
  );
...
test("app.js initial refresh reuses the live fast path and preserves control-plane fields", async () => {
  const payloads = new Map([
    ...
    ["/api/hosts", { hosts: [] }],
    ["/api/roster", { agents: [{ agent_slug: "security-reviewer" }] }],
  ...
  assert.equal(harness.api.state.config, null);
...
test("app.js control-plane refresh is single-flight and updates only current responses", async () => {
  const payloads = new Map([
    ...
    ["/api/roster", { agents: [{ agent_slug: "reviewer" }, { agent_slug: "tester" }] }],
...
test("app.js saves validated config only after the exact typed confirmation", async () => {
  ...
  const harness = createAppHarness(async (path, options) => {
    ...
    if (path === "/api/config") {
      return jsonResponse(200, {
        effective: { observability: { retention_days: 31 } },
...
test("app.js covers fallback helpers and forward modal focus trapping", async () => {
  ...
  assert.equal(harness.api.activeEvidenceKind(), "delegations");
...
test("app.js renders independent graphs and roster control refreshes", () => {
  ...
  assert.equal(harness.node("roster-count").textContent, "1");
});
...
test("app.js confirms, cancels, and completes roster and host actions", async () => {
  const calls = [];
  const payloads = new Map([
    ["/api/live?limit=100", {
  ...
  const harness = createAppHarness(async (path, options) => {
    calls.push({ path, options });
    if (path === "/api/roster/action" || path === "/api/hosts/toggle") {
      return jsonResponse(200, { ok: true });
    }
    return jsonResponse(200, payloads.get(path));
  });
  ...
  await answerConfirmation(
    harness,
    harness.api.toggleHost("codex", false),
    "DISABLE codex",
    false,
  );
  ...
  await answerConfirmation(
    failed,
    failed.api.toggleHost("claude", true),
    "ENABLE claude",
  );
  assert.match(failed.node("notice").textContent, /mutation failed/i);
});
...
test("app.js renders sparse and changing runtime evidence defensively", () => {
  ...
  harness.api.state.hosts = [
    ...
    { executable_discovered: true, host: "enabled", inspection_status: "complete", runtime_enabled: true },
    { executable_discovered: true, host: "disabled", inspection_status: "complete", runtime_enabled: false },
  ];
  harness.api.renderHosts();
...
test("app.js covers enabled-host success and tab/reset fallbacks", async () => {
  ...
  await answerConfirmation(host, host.api.toggleHost("claude", true), "ENABLE claude");
...
test("aborted mutations cannot render stale route, trim, config, roster, or host results", async () => {
  ...
  const suspendedHost = harness.api.toggleHost("codex", false);
  await Promise.resolve();
  harness.api.state.lifecycle.suspended = true;
  ...
  const host = harness.api.toggleHost("codex", false);
  await Promise.resolve();
  harness.node("confirmation-input").value = "DISABLE codex";
  harness.api.finishConfirmation(true);
  await Promise.resolve();
...
test("paged roster metadata drives global counts and accessible truncation disclosure", async () => {
  ...
  assert.equal(harness.api.state.overview.roster_count, 6);
  assert.equal(harness.node("metric-roster").textContent, "6");

  harness.api.state.activeView = "roster";
  harness.api.renderRoster();
  const pageStatus = harness.node("roster-page-status");
  assert.equal(harness.node("roster-count").textContent, "2 / 6");
  assert.equal(pageStatus.hidden, false);
  assert.match(pageStatus.textContent, /showing 2 of 6 active specialists/i);
  assert.match(pageStatus.textContent, /4 are not shown/i);
  assert.match(pageStatus.textContent, /\/api\/roster\?after=page%2F%3Cimg/i);
  ...
  harness.api.renderRoster();
  assert.match(pageStatus.textContent, /next_cursor is available/i);
  ...
  harness.api.renderRoster();
  assert.match(pageStatus.textContent, /limit=0/);
  ...
  assert.equal(optionalStatus.node("roster-count").textContent, "0");
});
