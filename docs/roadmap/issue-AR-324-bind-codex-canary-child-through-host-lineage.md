...
- Official `rust-v0.149.1` commit `ff29a443...9ea4` snapshots the process
  environment per hook session and constructs child lifecycle hooks from the
  child session. Its host-authored child `session_meta` also records the parent
  thread independently as `session_id`, `parent_thread_id`, and
...
history, and three agreeing parent fields. Use that host-authored parent UUID to
resolve exactly one live parent trace and exact accepted canary route. If the
request digest is present, require it to agree; never select a parent globally.
