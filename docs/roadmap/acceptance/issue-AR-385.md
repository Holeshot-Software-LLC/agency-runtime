...
| 3 | file | `_NominationAccumulator.parse records every reply it cannot read as missing_work_unit with a closed diagnosis: an unreadable unit row, a reply that is not a units object, and a repair answering for a unit outside the failed set; no path raises a bare error` | 2026-09-03 | `agency_runtime/core/workforce/inference.py:3128-3224` |
...
| 3 | file | `project_preflight_provider_attempts carries validation_failures, validation_reason_codes and the truncation object on each rejected attempt; it carries what the parser classified, and the parser classifies every rejection` | 2026-09-03 | `agency_runtime/core/preflight_failure.py:227-273` |
| 3 | test | `test_every_rejection_the_recruiter_parser_can_raise_is_classified drives thirteen malformed reply shapes through the parser and asserts every exception projects onto the receipt with a closed diagnosis` | 2026-09-03 | `tests/test_recruiter_reply_residue.py:409-450` |
| 3 | test | `test_an_empty_object_reply_is_repaired_before_the_turn_dies drives plan_and_staff_workforce through an empty-object recruiter reply and asserts the rejected attempt carries the units and the response-shape diagnosis` | 2026-09-03 | `tests/test_recruiter_reply_residue.py:453-481` |
| 3 | test | `test_a_verifier_rejection_projects_onto_the_attempt_row and test_verifier_rows_survive_the_re_projection_every_reader_applies assert the verifier's rows on the projected attempt, on write and on read` | 2026-09-03 | `tests/test_recruiter_reply_residue.py:532-609` |
| 3 | command-output | `eleven live turns on the ADR-0202 runtime: the two rejected attempts (206's not-a-units-object reply, 304's rows without scores) carry validation_failures and the diagnosis; 202's two verifier-rejected attempts were captured blank before the re-projection fix` | 2026-09-03 | `docs/roadmap/acceptance/evidence/AR-373-AR-385-residue-evidence-20260903.txt:50-79` |
| 3 | command-output | `202's captured verifier detail, projected with the final code: written rows and read-back rows identical; the verifier's first pass and the parser path it exposed are recorded with the response` | 2026-09-03 | `docs/roadmap/acceptance/evidence/AR-373-AR-385-residue-evidence-20260903.txt:81-108` |
