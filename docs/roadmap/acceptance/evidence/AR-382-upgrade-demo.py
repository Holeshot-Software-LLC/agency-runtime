...
for slug in sorted(KNOWN_CONTRACTORS_BY_SLUG):
    ...
    case = store.create_hiring_case(
        case_type="hire", proposed_slug=slug, work_unit_id=f"known-{slug}",
        request_hash=ki._request_hash(pkg), contract_evidence=doc,
        contract_hash=hashlib.sha256(json.dumps(doc, ensure_ascii=False,
    ...
    store.register_workforce_worker(
        agent_slug=slug, display_name=pkg.employment_contract.role, origin="agency",
        employment_class="contractor", agent_version_id=version_id,
        recruitment_contract=doc, relation="generated", hiring_case_id=case["id"])

seeded = store.get_workforce_workers_by_slugs(tuple(sorted(KNOWN_CONTRACTORS_BY_SLUG)), disabled_agents=())
print("seeded workers at:", sorted({w["current_version"].split("-")[1] for w in seeded.values()}))
r = ki.install_known_contractors(store)
print(f"installed={len(r.installed)} upgraded={len(r.upgraded)} already_current={len(r.existing)} preserved={len(r.preserved)}")
after = store.get_workforce_workers_by_slugs(tuple(sorted(KNOWN_CONTRACTORS_BY_SLUG)), disabled_agents=())
