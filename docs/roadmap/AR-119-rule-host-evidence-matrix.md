...
candidate_commit: 1bd7e37c6ea3be66488941392a956c3323b0472c
...
Candidate `1bd7e37c` advances `3269ff67` by one evidence document commit
(package tree unchanged): **R6 claude returns to proven at all four
layers on organically-recurred evidence** — a different project's session
ran the full hiring ladder at 05:05Z 2026-08-18 (critic approved, inline
dynamic audit approved, contract hashed, `deployment-readiness-reviewer`
filed `origin=agency` and dealt into its minting turn) and a later turn
of that session **reused the contractor from the pool at 06:29Z with zero
further hiring cases**. Seven of the eight rules are now proven at all
four layers on claude; R8 alone keeps R9 unproven there. Source
evaluation carries forward from `3269ff67` unchanged (docs-only delta).

Candidate `3269ff67` advances `f2f3ca88` by the v2/v3 canary instrument
(PRs #283/#284), the owner's small-unit policy in the complete-universe
judge prompt (PR #287), and the vision-loop session's documents; its
`agency_runtime/` tree is identical to merge `99a7b3ac` (verified by an
empty `git diff 99a7b3ac origin/main -- agency_runtime/`), which was
installed machine-wide on 2026-08-17 as runtime digest `cc478bc88258…` on
all three present hosts (one digest, AR-258), store schema 47. Source
evaluation: each constituent PR passed the repository's Linux CI quality
job at merge; the deterministic routing eval passed at this candidate
(`agency eval routing --json --no-details`, v1.4.0, exit 0); the
spawn-authority eval passed 5/5 with the installed launcher tree as the
analyzed package root; and the delta from `f2f3ca88`'s evaluated tree is
+299 inserted lines with zero deletions across nine files. The full local
gate suite was re-run green at this candidate before the advance merged.
The changes move one source anchor: R1 claude Implementation re-anchors to
`native_child_staffing.py:923-1084` (pure insertions shifted the cited
construct; verified byte-identical apart from one inserted capture block).

This candidate records the **first R1 and R4 Installed and Live layers
anywhere**: a live harness-spawned child in the loop session itself was
staffed just-in-time and its host-authored transcript carries the first
`[AGENCY INFERENCE TEAM v6]` envelope ever observed on this machine —
exact card hashes equal to the live roster row, `task_sha256` equal to an
independent recompute over the parent-recorded assignment, `launch_id`
equal to the parent's recorded tool_use id, and `runtime_digest` equal to
the installed digest, all pre-speech in record zero. R5 completes all
four layers (installed-tree separation re-run plus the live
host-originated spawn recorded-not-started). R2, R3, and R7 were
re-proven on this runtime the same night in a fresh real-profile
`claude -p` session (four cards selected and loaded with no narrowing,
pre-speech capsule, exact expiry across a resumed turn). R6 claude
demotes to prior-candidate context — its organic hiring occurrence binds
`2cd29815` and cannot be restaged on demand — which is the update
contract working as designed. The artifacts and their limitations are in
[the candidate evidence document](AR-119-99a7b3ac-live-evidence.md); the
runtime's own in-lifetime collector (canary-only) has still never minted
a `native_child_delivery_verifications` row, and the small-unit
acceptance draw remains open — tonight's three-run series was 0/3
provider-killed at parent preflight, recorded in the loop status
document.

Candidate `f2f3ca88` advances `9e29aabe` by the AR-255 P2 funded repair
(`966e8bae`), the hiring security-verdict repair (`e41ac039`), and the
overnight evidence documents; its `agency_runtime/` tree is identical to merge
`c77c67a4` (PR #275), which was installed machine-wide on 2026-08-17 as
runtime digest `2cd2981585843e…` on all three present hosts (one digest,
AR-258). The full production spine plus every matrix-cited suite passed
locally under `-W error` (1,462 tests, then 793 re-run after the hiring
repair), the Linux CI quality job passed on the exact head `8c8afcf6`, and
all 151 decision-conformance mutation snippets anchor exactly once. The full
mutation evaluator was **not** re-run overnight — the AR-257 sandbox
constraint stands — so this candidate carries the snippet-anchor check plus
Linux CI as its source-evaluation evidence and says so rather than silently
carrying the previous result forward.

This candidate records the matrix's **first Installed and Live layers**: R2,
R3, R6, and R7 are proven at all four layers on claude, and R5 gains its
Installed layer, from one fresh real-profile session on the installed
projection plus the installed-tree spawn-authority eval — the artifacts and
their limitations are in
[the candidate evidence document](AR-119-c77c67a4-live-evidence.md). R1 and
R4 remain blocked by the child judge, which the post-P2 series proved is
declining **on the merits** (per-run splits legacy / legacy /
`native_child_abstention_confirmed`; the confirmed row is the repair branch
executing live). R8 has a candidate artifact recorded but deliberately not
claimed. Codex activation was attempted only under the owner-authorized
autonomous bypass and was not proven (`preflight_failed` at the shared
recruiter stage, `trust_bypass_used: true` — bypass-derived, never attended
trust); zcode has no executable on this box, so its installed activation
needs the owner. openclaw and hermes remain absent by instruction; the
[owner-run verification packet](AR-119-openclaw-hermes-verification-packet.md)
is the runnable recipe for their cells.
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| R1 | claude | proven | proven | proven | proven | proven | Inference receipt joined to exact delivered card hashes | a live harness-spawned child's v6 envelope seals the decision id, provider identity, and exact card hash/version in one delivered object, and the store's decision, capture, and roster rows join it exactly | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r1-claude-live` | Single occurrence; the envelope's provider_receipt_digest was not independently recomputed |
...
| R2 | claude | proven | proven | proven | proven | proven | Native primary-caller artifact containing selected cards before first caller speech | fresh real-profile session on the installed `cc478bc88258…` projection: the persisted UserPromptSubmit capsule carries whole instruction bodies, attached before the first caller speech, joined to the accepted decision and four specialists_loaded rows, zero delegation events, selected set equal to loaded set | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r2-claude-live` | One turn proves activation and delivery, not a rate |
...
| R3 | claude | proven | proven | proven | proven | proven | Native primary-caller artifact with multiple compatible card hashes before first caller speech | the same live turn delivered four compatible cards' whole instruction bodies in one capsule, pre-speech, each with its own specialists_loaded row on the trace, with no selected-to-loaded narrowing | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r3-claude-live` | One turn, not a rate |
...
| R4 | claude | proven | proven | proven | proven | proven | Correlated native child artifact with exact card hashes before first speech | the child transcript's record zero carries the v6 envelope pre-speech: exact card hash equal to the roster row, task hash equal to an independent recompute of the parent-recorded assignment, launch id equal to the parent's tool_use id, runtime digest equal to the installed digest | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r4-claude-live` | Single child; a ~2,000-char research unit (the small-unit draw stays open); the runtime's canary-only collector never evaluated it, so the verification is reproduced from retained artifacts instead of a collector row |
...
| R5 | claude | proven | proven | proven | proven | proven | Source separation of process origin from worker origin plus native spawn-origin artifact | the spawn-authority eval re-passed 5/5 with the analyzed package root literally the installed launcher tree, and a live host-originated spawn was recorded rather than started: parent tool_use, delegation event, and the child staffing decision, with Agency starting no process | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r5-claude-live` | The separation is host-neutral, so one measurement covers it; one live spawn; the negative half rests on the installed separation rather than a whole-turn seam sweep |
...
| R6 | claude | proven | proven | proven | proven | proven | Inference hiring receipt independent critic receipt immutable identity and host-backed use | an organic turn on the installed runtime hired `deployment-readiness-reviewer` (critic approved, inline audit approved, contract hashed), filed it origin=agency, dealt it into the minting turn, and a later turn reused it from the pool with zero further hiring cases | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r6-claude-live` | Same-session reuse; cross-session reuse unmeasured; the dynamic-hire audit is inline rather than a separate security-review row |
...
| R7 | claude | proven | proven | proven | proven | proven | Same identity observed in one turn and absent from the next turn | two live turns in one `cc478bc88258…` session: all four turn-1 cards named in turn 2's `[AGENCY SPECIALIST EXPIRY]` notice pre-speech, none re-delivered, and every `specialists_loaded.expired_at` equals run 1's `ended_at` exactly | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r7-claude-live` | One two-turn observation; the expiry notice rides the next turn's capsule, so an empty-context turn would not carry it |
...
| R9 | claude | unproven | proven | proven | unproven | unproven | Aggregate of every R1 through R8 cell under one exact candidate identity | R1 through R7 are proven at all four layers on claude at this candidate; R8 Installed/Live alone await a clean unstaffed-turn publication artifact (owner-gated capture decision) | 2026-08-18 | `docs/roadmap/AR-119-rule-host-evidence-matrix.md` | Seven of eight rules fully proven on this host; the R8 gap keeps the aggregate unproven |
...
|---|---|---|---|---|---|---|---|
| R1 | claude | Implementation | proven | source | complete-universe inference selection with exact whole-team validation | 2026-08-18 | `agency_runtime/core/native_child_staffing.py:923-1084` |
...
| R1 | claude | Installed | proven | installed-host | the installed projection's hook staffed a live harness-spawned child and rendered the v6 envelope naming the installed digest from inside the artifact | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r1-claude-installed` |
| R1 | claude | Live | proven | live-host | the live v6 envelope seals decision id, provider identity, and exact card hash/version; decision, capture, and roster rows join it exactly | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r1-claude-live` |
| R2 | claude | Installed | proven | installed-host | fresh-session activation of the installed projection delivered the capsule into the caller's turn, host-artifact-supported | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r2-claude-installed` |
| R2 | claude | Live | proven | live-host | live real-profile turn: whole instruction bodies pre-speech, store join, zero delegations, no narrowing | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r2-claude-live` |
| R3 | claude | Installed | proven | installed-host | the installed projection delivered a multi-card capsule into one caller turn pre-speech | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r3-claude-installed` |
| R3 | claude | Live | proven | live-host | four compatible cards' whole bodies in one live turn, each with its own specialists_loaded row | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r3-claude-live` |
| R4 | claude | Installed | proven | installed-host | the installed projection staffed a native child just-in-time; the envelope's runtime digest equals the installed digest | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r4-claude-installed` |
| R4 | claude | Live | proven | live-host | child record zero carries the v6 envelope pre-speech with exact card hashes, task hash, and launch id verified across parent artifact, child artifact, and store | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r4-claude-live` |
| R5 | claude | Installed | proven | installed-host | spawn-authority eval re-passed 5/5 with the installed launcher tree as the analyzed package root, import path asserted in-process | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r5-claude-installed` |
| R5 | claude | Live | proven | live-host | a live host-originated spawn recorded rather than started: parent tool_use, delegation event, child staffing decision, no Agency-started process | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r5-claude-live` |
| R6 | claude | Installed | proven | installed-host | the installed projection ran the whole hiring ladder organically: applied case with critic approval, inline dynamic audit approved, contract hashed, origin=agency contractor filed | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r6-claude-installed` |
| R6 | claude | Live | proven | live-host | the minted card was dealt into the very turn whose gap created it, and a later turn reused it from the pool with zero further hiring cases | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r6-claude-live` |
| R7 | claude | Installed | proven | installed-host | the installed projection expired turn 1's cards at run end and announced them to turn 2 | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r7-claude-installed` |
| R7 | claude | Live | proven | live-host | turn 2's capsule names all four expired cards pre-speech, none re-delivered, expired_at equals run 1's end exactly | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r7-claude-live` |
