...
(package tree unchanged): R6 claude was advanced to proven here on
organically-recurred hiring evidence and **retracted the same day** by
adversarial review, together with R5 Installed and Live: see the
retraction sections in the candidate evidence document. The ladder
evidence itself is sound — the mint ran on the exact installed candidate
with a real cross-provider security review — but R6's **host-backed use**
clause is contradicted by the host's own headers, and R5's two layers
rested on an eval that cannot evidence which tree it read and on a
"started no process" sentence its own provider row falsifies. A second review then retracted R1
and R4 as well: their envelope verifies to the byte, but delivery ran
**one in fourteen** harness-spawned children at this candidate, and the
promotion had reversed this session's own recorded conservative reading
without writing a decision. **Three of eight rules (R2, R3, R7) stand
proven at all four layers on claude**, and those rest on two live events;
R1, R4, R5, R6 and R8 keep R9 unproven there. The night's genuine gain is
narrower and still real: the first verified v6 delivery to a
harness-spawned child, recorded as an existence proof. Source evaluation carries
forward from `3269ff67` unchanged (docs-only delta).
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| R1 | claude | unproven | proven | proven | unproven | unproven | Inference receipt joined to exact delivered card hashes | claimed 2026-08-18 and RETRACTED the same day: the single live v6 envelope verifies to the byte, but it is one delivery in fourteen harness-spawned children at this candidate, the promotion reversed this session's own recorded conservative reading with no decision written, and Installed is not independent of Live (same envelope) | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r4-and-r1-claude--retracted-2026-08-18-an-existence-proof-not-a-proven-cell` | Existence proof only; needs a collector-minted proof or a measured delivery rate with a receipt bound to every non-delivery |
...
| R4 | claude | unproven | proven | proven | unproven | unproven | Correlated native child artifact with exact card hashes before first speech | claimed 2026-08-18 and RETRACTED the same day: the artifact is real and fully bound (launch id, independent task-hash recompute, byte-identical card body, installed digest, pre-speech) but delivery ran 1-in-14 at this candidate with three unstaffed siblings in the measuring session carrying no receipt, `native_child_delivery_verifications` is still empty, and one card is not the plural team demanded of other hosts | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r4-and-r1-claude--retracted-2026-08-18-an-existence-proof-not-a-proven-cell` | Existence proof only; the matrix's own rule is that a collector-minted host-artifact proof is the only thing that satisfies Rule 4 |
...
| R5 | claude | unproven | proven | proven | unproven | unproven | Source separation of process origin from worker origin plus native spawn-origin artifact | Installed and Live were claimed 2026-08-18 and RETRACTED the same day: the eval emits no package path and `modules: 295` is shared by both checkouts and eight launcher trees, so it cannot evidence which tree it read; the Live claim's "Agency started no process" is falsified by its own turn's `codex-subscription` CLI provider call (11.85 s), and a delegation row carries no absence evidence | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r5-claude-installed--retracted-2026-08-18` | Retraction recorded in full; Installed needs the launcher's `-I -S` bootstrap with retained stdout, Live needs a whole-turn seam sweep with a positive control |
...
| R6 | claude | unproven | proven | proven | unproven | unproven | Inference hiring receipt independent critic receipt immutable identity and host-backed use | claimed 2026-08-18 and RETRACTED the same day: the ladder itself is solid on the exact installed candidate (real security review `verdict: safe`, cross-provider critic, contractor filed, last hiring row in the table, two provably distinct turns), but the authority's **host-backed use** clause fails — the host's own headers on both turns read `agency-steward` / `Recruited via: none`, the only text naming the contractor as loaded is Agency-authored, and both runs terminated `response_invalid` | 2026-08-18 | `docs/roadmap/AR-119-99a7b3ac-live-evidence.md#r6-claude-installed-and-live--not-claimed-the-ladder-half-holds-the-use-half-fails` | Needs one host turn whose own final header names the minted contractor, on a run that does not terminate `response_invalid` |
...
| R9 | claude | unproven | proven | proven | unproven | unproven | Aggregate of every R1 through R8 cell under one exact candidate identity | R2, R3 and R7 are proven at all four layers on claude at this candidate; R1, R4, R5 and R6 Installed/Live were claimed and retracted on 2026-08-18 by adversarial review, and R8 awaits an owner-gated capture decision | 2026-08-18 | `docs/roadmap/AR-119-rule-host-evidence-matrix.md` | Three of eight rules fully proven on this host; and those three rest on two live events, not six |
