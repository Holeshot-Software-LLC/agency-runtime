...
candidate_commit: f2f3ca88dbe4bc9adeb636a028f615c5d4886152
...
| R2 | claude | proven | proven | proven | proven | proven | Native primary-caller artifact containing selected cards before first caller speech | fresh real-profile session on the installed `2cd29815…` projection: the transcript's UserPromptSubmit capsule carries the selected card's whole instruction body, attached before the first caller speech, joined to the accepted decision and specialists_loaded rows, with zero delegation events | 2026-08-17 | `tests/test_parent_caller_card_delivery.py:189-213` | One turn proves activation and delivery, not a rate; the decision selected seven specialists and the artifact proves the four loaded |
...
| R3 | claude | proven | proven | proven | proven | proven | Native primary-caller artifact with multiple compatible card hashes before first caller speech | the same live turn delivered four compatible cards' whole instruction bodies in one capsule, pre-speech, each with its own specialists_loaded row on the trace | 2026-08-17 | `tests/test_parent_caller_card_delivery.py:215-236` | One turn, not a rate; the 7-selected/4-loaded narrowing is recorded in the candidate evidence document |
...
| R5 | claude | unproven | proven | proven | proven | unproven | Source separation of process origin from worker origin plus native spawn-origin artifact | the spawn-authority eval passed 5/5 with the analyzed package root literally the installed launcher tree, both injected-violation controls detected | 2026-08-17 | `agency_runtime/core/evals/spawn_authority.py:48-318` | Installed layer only: the separation is host-neutral, so one measurement covers the layer, and the Live native spawn-origin artifact is still open at this candidate |
...
| R6 | claude | proven | proven | proven | proven | proven | Inference hiring receipt independent critic receipt immutable identity and host-backed use | an organic live turn minted `function-naming-advisor` mid-turn (hiring case applied, cross-provider critic, security review recorded safe), filed it origin=agency, dealt it into the very turn whose gap created it in the host artifact, and the next same-domain turn reused it from the pool with no further hiring case | 2026-08-17 | `tests/test_contractor_minting_host_parity.py:297-333` | Single organic occurrence; the reuse turn ran in the same session, and cross-session reuse was not measured |
...
| R7 | claude | proven | proven | proven | proven | proven | Same identity observed in one turn and absent from the next turn | two live turns in one session: all four turn-1 cards named in turn 2's `[AGENCY SPECIALIST EXPIRY]` notice, none re-delivered, and every `specialists_loaded.expired_at` equals run 1's `ended_at` exactly | 2026-08-17 | `agency_runtime/core/evals/host_parity.py:231-293` | One two-turn observation; the expiry notice rides the next turn's capsule, so an empty-context turn would not carry it |
...
|---|---|---|---|---|---|---|---|
| R1 | claude | Implementation | proven | source | complete-universe inference selection with exact whole-team validation | 2026-08-12 | `agency_runtime/core/native_child_staffing.py:876-1031` |
...
| R2 | claude | Installed | proven | installed-host | fresh-session activation of the installed projection delivered the capsule into the caller's turn, host-artifact-supported | 2026-08-17 | `docs/roadmap/AR-119-c77c67a4-live-evidence.md#r2-claude-installed` |
| R2 | claude | Live | proven | live-host | live real-profile turn: whole instruction bodies pre-speech, store join, zero delegations | 2026-08-17 | `docs/roadmap/AR-119-c77c67a4-live-evidence.md#r2-claude-live` |
| R3 | claude | Installed | proven | installed-host | the installed projection delivered a multi-card capsule into one caller turn pre-speech | 2026-08-17 | `docs/roadmap/AR-119-c77c67a4-live-evidence.md#r3-claude-installed` |
| R3 | claude | Live | proven | live-host | four compatible cards' whole bodies in one live turn, each with its own specialists_loaded row | 2026-08-17 | `docs/roadmap/AR-119-c77c67a4-live-evidence.md#r3-claude-live` |
| R5 | claude | Installed | proven | installed-host | spawn-authority eval passed 5/5 with the installed launcher tree as the analyzed package root | 2026-08-17 | `docs/roadmap/AR-119-c77c67a4-live-evidence.md#r5-claude-installed` |
| R6 | claude | Installed | proven | installed-host | the installed projection ran the whole hiring ladder in a real turn: applied case, cross-provider critic, safe security review, origin=agency contractor | 2026-08-17 | `docs/roadmap/AR-119-c77c67a4-live-evidence.md#r6-claude-installed` |
| R6 | claude | Live | proven | live-host | a live gap minted function-naming-advisor mid-turn, dealt it into that turn's capsule, and the next same-domain turn reused it with no further hiring case | 2026-08-17 | `docs/roadmap/AR-119-c77c67a4-live-evidence.md#r6-claude-live` |
| R7 | claude | Installed | proven | installed-host | the installed projection expired turn 1's cards at run end and announced them to turn 2 | 2026-08-17 | `docs/roadmap/AR-119-c77c67a4-live-evidence.md#r7-claude-installed` |
| R7 | claude | Live | proven | live-host | turn 2's capsule names all four expired cards, none re-delivered, expired_at equals run 1's end | 2026-08-17 | `docs/roadmap/AR-119-c77c67a4-live-evidence.md#r7-claude-live` |
