...
cases and all 188 UI tests pass. Its four original isolated criteria remain.
...
| 16 | AR-154 | In verification: existing initial-page schema/cursor/revision admission is fail-closed. Twelve new full/control refresh cases directly retain last-good data and revisions for missing first cursor/revision in roster, snapshots and reviews. Focused cursor/activity/observation 13 pass; UI 188 pass current production floors. Python/source receipt reuse is explicit. No runtime or criterion change. | Owned branch; four isolated criteria pending, count unchanged. |
