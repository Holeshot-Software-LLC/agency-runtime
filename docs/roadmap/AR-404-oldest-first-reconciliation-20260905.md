...
spine 1085/three skips and UI 188 pass; exact wheel QA remains before publication.
...
| 20 | AR-158 | Existing MCP/HTTP exact selectors verified. Hook observation proof restored on the current three-host R8 failure test; no retired denial restored. Store slow/busy fixtures inject matching unrelated events, busy selection requires exact request ID, and privacy checks cover all envelopes. Focused 13 pass, ten five-case repeats all pass, complete five-module package 135/five skips. Runtime unchanged; scoped spine/UI/wheel reuse. Only criterion 7 reconciled under ADR-0105. | Owned branch; seven isolated verdicts pending; no count change yet. |
