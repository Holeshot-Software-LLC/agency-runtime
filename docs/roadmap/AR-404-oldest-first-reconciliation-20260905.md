...
| 47 | AR-194 | Existing cross-interpreter inspection and strict execution preparation remain implemented. Fresh 116 focused tests/42 deselections pass; actual installed Linux systemd status is owned/current/active/reachable with no repair needed. Four inspection/CLI modules match source. ADR-0117 removes only the obsolete AR-196 presence dependency reciprocally; all five original criterion states remain. | Retain in_progress for native Windows stale-task/runtime repair and post-repair reachability. Branch-only checkpoint, publication reserved after AR-190/191/192 (orders44/45/46). No owner mutation, native Windows execution, acceptance verdict or queue-count reduction. |
