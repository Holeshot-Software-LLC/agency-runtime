...
A later current-profile review of exact wheel `af892ae` rendered the live Store
and truthful Codex `activation-required` proof, but exposed a large blank mobile
header gap at 390 x 844. The cause was the desktop heading's 280 px flex basis
becoming vertical after the mobile layout switched to a column. Commit
`9aa317c` resets that basis only at the breakpoint. Live source rechecks now
measure a 56 px heading instead of 280 px, preserve the desktop layout, and
emit zero browser warnings or errors. This is a source repair; the next exact
packaged candidate still needs the same bounded visual recheck.
...
106-test dashboard gate passes at 98.74 percent lines, 90.70 percent branches,
and 97.98 percent functions, again without changing the coverage floors. The
mobile breakpoint regression is source-fixed and browser-verified; a fresh
exact packaged-candidate desktop/mobile rerun remains the AR-138 exit gate.
