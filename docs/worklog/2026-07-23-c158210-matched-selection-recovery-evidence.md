...
superseded_by: null
type: worklog
commit: c15821093c626fde1bf9ba22d563efb5fd162956
short: c158210
date: 2026-07-23
pr: null
related_issues:
...
whether its sole Agency failure represented a governed general semantic defect
...
The sole complete-run Agency failure received an immediate bounded matched
confirmation under the same controls. Selection safety recovered on that first
rerun with its exact specialist. AR-119 records every stream receipt,
aggregate, fingerprint, provider/model binding, and the exact 19-line and
one-line compact projections.

## Challenges encountered

The complete corpus produced one selection-safety confidence and margin
abstention. Three upstream arms returned malformed assignment or disabled-
shadow contracts, so the complete benchmark remained invalid and none was
scored as an upstream loss.

Selection safety recovered immediately with `selection-safety-critic`,
complete typed coverage, and a valid matched benchmark. This separated
configured-provider plan-shape variance from a repeatable governed selection-
semantic defect without weakening any fairness or safety gate.

## Decisions and alternatives

No product or policy code changed. The sole complete-run Agency failure passed
...
- The complete process finished in 418.513 seconds, emitted 1,185,217 stdout
  bytes and zero stderr bytes, and preserved independently reverified stream
  hashes.
- Agency passed 18/19 with F1 0.916667, 18/19 complete typed coverage, and
  zero forbidden, ineligible, or conflict selections.
- The selection-safety-only confirmation passed with F1 1.000000, complete
  typed coverage, 7523.085 ms latency, and zero safety defects.
- All 40 captured arms preserved the expected provider, requested and actual
...
- Metadata, policy availability, worklog-currentness, documentation validation,
  exact 19/1 projection comparisons against the raw captures, capture-hash
