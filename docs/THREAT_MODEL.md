...
| Codex child rewrite authorization | A Codex collaboration message is rewritable only when a version-pinned, bounded scan of the canonical private parent transcript binds the exact session, turn, call, namespace, full arguments, and explicit empty encrypted-argument marker. Lexical and descriptor identities, private permissions, link and reparse rejection, exact record digests, repeated reattestation, and an atomic one-use Store guard reject substitution, TOCTOU, concurrency, and replay. Missing, null, nonempty, ambiguous, completed, oversized, unsupported-version, or schema-drift evidence fails open unstaffed. This authority permits only the rewrite; it cannot replace the host-authored child artifact required to prove delivery. |
...
- Host-authored child artifacts establish the evidence authority boundary, not
  ...
  a conditional marked plaintext path. Until ADR-0159's exact-call attestation
  and adversarial simulation are implemented, the result remains negative in
  the evidence matrix. Temporal correlation, task labels, plaintext appearance,
  Store lifecycle rows, or generic MCP attribution cannot fill that gap. Each
  supported host still needs its own current installed and live artifact,
  and no host's evidence substitutes for another's.
