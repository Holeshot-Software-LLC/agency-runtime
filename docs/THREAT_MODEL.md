...
| Codex child rewrite authorization | A Codex collaboration message is rewritable only when a version-pinned, bounded scan of the canonical private parent transcript binds the exact session, turn, call, namespace, full arguments, and explicit empty encrypted-argument marker. Canonical padded paths, non-nil observed UUID domains, lexical and descriptor identities, private permissions, link and reparse rejection, exact record digests, repeated reattestation, and an atomic one-use Store guard reject substitution, TOCTOU, concurrency, and replay. Accepted ancestry must be fully materialized in the same rollout; authentic one-record fork prefixes remain unsupported until a trusted cross-file parent/root verifier exists. Missing, null, nonempty, ambiguous, completed, oversized, unsupported-version, unsupported-ancestry, or schema-drift evidence fails open unstaffed. This authority permits only the rewrite; it cannot replace the host-authored child artifact required to prove delivery. |
