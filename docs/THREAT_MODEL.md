...
| Prepared operator presence | Three exact Windows 11 x64 mutation slices are positive. Roster rollback captures exact Store, projection, authority, and workforce primitives, re-reads them under `BEGIN IMMEDIATE`, and commits once. Existing Codex refresh captures exact config/database/control generations, current target tree, candidate plan and launcher, Codex executable/environment/version, and strict marketplace/plugin inventory; after verification it re-prepares under the shared host-integrations lock, atomically publishes the target, removes and re-adds the plugin, and proves both filesystem and native postconditions. Its compensation path conditionally restores the prior tree and registration and reports manual recovery when exact proof is unavailable. Host-integration uninstall uses the distinct `uninstall.host-integrations.v1` action and binds one operation UUID, selector, canonical hosts/transitions, outer plan and per-host hashes, exact retained destinations, and fixed preservation/recovery policies; it re-plans under the same lifecycle lock before detaching or retiring anything. None of these paths exposes public prepare/commit methods, accepts an injected verifier/boolean/receipt, or exports the native result. The packaged app-owned GUI invokes `IUserConsentVerifierInterop::RequestVerificationForWindowAsync`; only exact nonce-bound verified output from the identity-pinned contained helper continues. Denial, substitution, race, malformed result, or verified failure cannot be promoted to success. A separate parser-marked, closed-world Codex activation-verification shape is evidence-producing rather than installation-authoritative: it branches before generic install work; requires an existing current WAL Store in both coordinator and spawned hooks; permits only bounded canary evidence and attestation replacement; and suppresses catalog bootstrap/reconciliation and dynamic hiring so it cannot change configuration, controls, roster/workforce, dashboard, adapter, registration, or trust state. A missing Codex installation, crash-durable automatic recovery, every other operation, and unsupported platforms remain fail closed. |
