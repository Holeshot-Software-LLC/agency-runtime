...
| Native processes and delegation | Argument arrays without a shell; absolute-only executable discovery that ignores empty, dot, relative, and current-directory `PATH` entries; canonical regular targets outside the target repository; Windows link/reparse rejection and POSIX launcher-symlink canonicalization; frozen executable, interpreter, and wrapper identities revalidated immediately before process creation; minimal allowlisted environment; task content through standard input when supported; bounded output and time; Windows Job Objects or POSIX process groups; bounded versioned unit-aware specialist assignment; validated dependency DAG; stable event-driven ready queue; strict failure-gated successors; mutating Git operations suppress hooks, inherited Git configuration, fsmonitor, executable filters, merge drivers, and text converters; merge only after proven success. Restricted Codex Windows scratch requires a file-ID-bound host capability under the owner-private visualization namespace, an authoritative effective-token logon SID, protected child DACLs, and link-safe identity-bound cleanup. Process-local authority never crosses `exec`: every child independently reattests only its exact randomized thread-bound allocation against the canonical host marker, root/parent identity, DACL, and mutation access; ambiguous roots and repository fallbacks fail closed. |
...
| Owned child processes | Shell-free bounded argv and I/O; isolated process groups or Windows Job Objects; deterministic timeout cleanup; Windows system helpers resolve through validated allowlisted native paths rather than CWD or `PATH`. |
