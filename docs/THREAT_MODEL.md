...
| Prepared operator presence | Exact roster rollback on Windows 11 x64 is the first and only positive slice. The sole public Store coordinator captures exact built-in primitives for config/database lexical and file identity, generation, slug, current projection, target revision, activation authority, and workforce/effective-contract identity. No public prepare/commit methods, injected verifier, boolean, or receipt can authorize it. A packaged app-owned GUI invokes `IUserConsentVerifierInterop::RequestVerificationForWindowAsync`; only the exact nonce-bound verified output from the identity-pinned contained helper continues. The same captured tuple is compared after native return and fully re-read under `BEGIN IMMEDIATE` on the same Store before a single commit. Bundled authority binds the manifest/revision; snapshot authority binds candidate, audit, status, snapshot, and import-event records. Workforce authority binds explicit absence or the complete ordered projection chain, capped at 1,024 shared-8-KiB contracts, with exact production authority, parent hashes, document hashes, and worker/agent/version/origin identity. An eligible absent upstream lineage binds its deterministic generated contract. The target routing/workforce contract is restored while current employment and standing are preserved. Denial, substitution, race, replay, malformed result, or apply failure commits no rollback effect. Success audit records only target, authority, workforce, mechanism, and pinned-helper provenance—never nonce, stdout, result, or receipt. All other operations and platforms fail closed. |
...
- The owner dashboard bearer is not proof of human presence when a model can
  ...
  exact roster rollback on Windows 11 x64 now has one OS-backed, non-exporting
  prepared path, but no dashboard or adjacent mutation inherits that authority.
