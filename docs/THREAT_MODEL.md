...
| Supply chain | Minimal runtime dependency; pinned build, audit, and workflow tools; immutable GitHub Action SHAs; wheel/sdist parity checks; exact installed-runtime vulnerability audit; Bandit; local CodeQL analysis with native upload or retained SARIF; native dependency-diff review when GitHub exposes it; offline workflow security linting; no credential persistence in checkout steps. |
