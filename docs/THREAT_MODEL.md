...
For specialist staffing and native-child delivery, the
[founding nine-rule vision](roadmap/AR-119-founding-vision.md) defines the
product authority and the
[rule and host evidence matrix](roadmap/AR-119-rule-host-evidence-matrix.md)
defines current maturity. Historical delegation mechanisms cannot override
either source.
...
| Inference staffing decisions and card composition | Bound to the exact request, configuration, roster, recalled candidates, hard-eligibility results, inference revision, and selected card hashes. Inference alone chooses specialists or contractors. Deterministic code may recall, enforce hard eligibility and safety, validate, budget, and correlate; it may not choose, replace, broaden, narrow, or erase the staffing decision. No conflicting or ineligible card is hydrated merely because it scored highly. |
| Staffing and outcome evidence | Correlated to canonical trace, turn, caller, native parent-child relation, selected card hashes, installed candidate, and distinct outcome records. Model-authored claims and Agency-authored Store rows are diagnostic, not native-child delivery authority. |
| Native child delivery authority | Agency does not decide whether to spawn. Only an artifact written by the native host, containing the exact delivered card hashes before first child speech and correlated to the host-created child, can originate a Rule-4 delivery claim. A missing or opaque host channel remains unproven or negative rather than being replaced by an Agency receipt. |
...
| Boundary | Controls |
|---|---|
| Context-dependent routing | One exact prior substantive row in the same session and host supplies a transcript-free subject capsule with closed identifiers and governed card metadata. Planner and recruiter receive it as a separate untrusted field used only to resolve the current request's referent. Prior messages, request summaries, outcomes, resources, paths, risks, acceptance prose, and final responses remain excluded even when content capture is enabled. Historical identities never force selection or grant authority. Planner/recruiter cache identities and the durable receipt bind the context digest; the ready transaction reselects the source and checks its sequence, evidence revision, recipe digest, context digest, and roster generation. A mismatch fails without publishing stale context. Advisory gap hiring may mutate the internal workforce, but advisory projection still forbids native-child, workspace-write, and external-write authority. |
...
| Selection and prompt composition | Turn intent, expertise selection, and native spawn topology are separate authorities. Hard host, platform, tool, permission, activation, and safety filters may reject ineligible inputs, but inference is the only specialist or contractor chooser. Recruiter inference defines the ideal owner from an open-ended pool and may select multiple compatible cards; deterministic code may recall candidates, validate the exact inference decision, enforce conflicts and hard eligibility, budget, and correlate, but cannot select, replace, broaden, narrow, or erase it. Inference receives bounded structured cards, never full prompt bodies. Selected cards load into the existing caller for that turn and expire at its boundary. Agency never asks the host to spawn; a native child is staffed only after the harness independently creates it. A genuine roster gap may trigger an inference-designed, narrowly scoped contractor and a safety interview; deterministic review may reject an unsafe proposal but cannot author or choose the replacement. If staffing is unavailable or no card is accepted, the host answers as a generalist with visible diagnostic evidence. Agency unavailability never withholds the turn. A verifier's definite negative and an unreadable malformed `Stop` envelope remain the deliberate blocking paths because they are established integrity findings, not availability failures. |
| Codex child rewrite authorization | A Codex collaboration message is rewritable only when a version-pinned, bounded scan of canonical private host transcripts binds the exact session, turn, call, namespace, full arguments, and explicit empty encrypted-argument marker on the current authorization call. The sealed v3 attestation chooses an atomic profile only from exact transcript metadata: the existing CLI `0.147.0` TUI/exec profiles remain unchanged, and a separate Desktop `0.147.0-alpha.6.6` profile accepts one exact root plus only its 13 observed depth-one/depth-two V2 child lineage tuples; eight unobserved cross-products, disabled guardians, and exec depth-two/deeper fail open unstaffed. Cross-file ancestry requires unique canonical owner files, bounded UTC-day plus-or-minus-one lookup with independent offsets, both depth-two edges, exact adjacent causal records, sealed copies/prefixes/files/profile/currentness, and a 64 MiB aggregate bound. Desktop additionally seals each exact call, direct start event, and compact function output; copied history corroborates but never owns ancestry. Ancestor causal calls accept only the observed ordinary schema or that exact schema plus an empty marker; the current call still mandates the marker. Canonical paths and UUID domains, lexical and descriptor identities, private permissions, link/reparse rejection, repeated reattestation, and an atomic one-use Store guard reject substitution, TOCTOU, concurrency, and replay. Missing, null, nonempty, ambiguous, completed, oversized, unsupported-version, unsupported-ancestry, or schema-drift evidence fails open unstaffed. This authority permits only the rewrite; it cannot replace the host-authored child artifact required to prove delivery. |
...
| Native processes and delegation | Argument arrays without a shell; absolute-only executable discovery that ignores empty, dot, relative, and current-directory `PATH` entries; inert repository-marker discovery before the first Git call excludes every containing repository ancestor from both search and final lexical/resolved candidate acceptance; repository-independent host lifecycle commands prepare and execute from one owner-private working directory while retaining every ambient marker-derived repository boundary; canonical regular targets outside the target repository; Windows link/reparse rejection and POSIX launcher-symlink canonicalization; every executable or wrapper artifact occurs at one exact ordered argv position, with the first identity covering `argv[0]`; frozen ephemeral or persistent identities cover those same paths and are revalidated immediately before process creation; minimal allowlisted environment; task content through standard input when supported; bounded output and time; kill-on-close Windows Job membership assigned atomically through `STARTUPINFOEX` at process creation, or a dedicated non-dumpable Linux subreaper with a pre-opened `/proc` children descriptor and pidfd signaling. Before `exec`, the Linux target enters a separate session, arms parent-death signaling, and inherits a `no_new_privs` seccomp policy denying supervisor-targeted signals, queued signals, pidfd acquisition, resource-limit changes, scheduler and affinity mutations, priority changes, and I/O-priority changes while retaining normal own-child operations. A private policy acknowledgement precedes external `READY`, but target code remains blocked until the parent durably owns containment state and every I/O worker, then sends exact `GO\n` plus EOF as a one-way commit. Exact cancellation prevents pre-commit execution; ambiguous post-commit interruption drains the full tree before propagation. Exactly one final `COMPLETE` after descendant drain and resource close is required. Missing, malformed, duplicate, out-of-order, or truncated receipts and unavailable strong-containment primitives fail closed. Native harnesses retain sole spawn authority; Agency cards and staffing decisions contain no spawn instruction. When a harness creates a caller, Agency may load the exact inference-selected compatible card set into that caller, but only the host-authored child artifact can prove delivery. Mutating Git operations suppress hooks, inherited Git configuration, fsmonitor, executable filters, merge drivers, text converters, and pathspec magic; merge only after proven success. Restricted Codex Windows scratch requires a file-ID-bound host capability under the owner-private visualization namespace, an authoritative effective-token logon SID, protected child DACLs, and link-safe identity-bound cleanup. Process-local authority never crosses `exec`: every child independently reattests only its exact randomized thread-bound allocation against the canonical host marker, root/parent identity, DACL, and mutation access; ambiguous roots and repository fallbacks fail closed. |
| Evidence, finalization, and promotion | Fresh printable trace and session IDs are bounded to 512 UTF-8 bytes before lookup or indexed persistence; privacy-safe request fingerprints and durable typed turn classifications bind the exact inference decision, selected card hashes, caller, installed candidate, and terminal outcome. Exact Agency and host-native tool allowlists replace namespace-suffix trust; duplicate, missing, stale, or ambiguous joins fail. Selection is not load evidence. For a native child, only the host-authored artifact may originate a Rule-4 delivery verdict, and it must contain every exact selected card hash before first child speech plus the native parent-child correlation. Agency Store `specialist_load` and lifecycle rows remain diagnostic projections and cannot promote an unproven delivery. Final response integrity still binds the exact accepted digest, provider telemetry, and monotonic terminal outcome without a hidden retry. Contractor production outcomes require a host-backed producer artifact and a separate verifier artifact from an independently inference-selected verifier; producer and verifier identities must remain distinct. Automatic promotion requires three distinct accepted outcome identities after the seven-day review window, all joined to the same contractor revision and valid installed evidence. Duplicate outcomes, self-review, Store assignment alone, elapsed time alone, and manual status mutation cannot satisfy or accelerate promotion. Protocol input and output remain strict finite JSON; LiteLLM provenance is granted only by the callback-owned Store ingress; final headers reconcile against canonical SQLite diagnostics without treating them as native delivery proof. |
| Installation and canaries | Canonical bundle digest and exact managed-tree comparison; unexpected files force replacement; owner-private staging; POSIX Hermes pre-manifests a mode-0500 `__pycache__` namespace with one mode-0400 ownership marker so normal imports cannot introduce executable cache drift while the private plugin root remains movable; backups and rollback; native inventory postconditions; interpreter plus package-bootstrap lexical/resolved metadata/content identities persisted in the managed install manifest; launcher drift or unproven identity makes maturity stale and blocks registration; Windows permission setup rejects restricted or indeterminate tokens before DACL mutation; scheduled-task definitions use BOM-bearing UTF-16 input, token-SID-bound identity, bounded Base64/UTF-8 COM inspection, strict schema normalization, semantic execution checks, and exact pre-mutation requery; isolated canary homes receive an owner-only directory policy before credentials exist; bounded link-resistant authentication copy hardens the empty destination before writing; a per-harness child-judge pin narrows Agency canaries to one exact configured CLI provider or supported inference profile with no fallback, requires a unique identity and a safe available endpoint, requires the disposable environment to project that same identity, and copies cross-provider CLI authentication only into a second owner-private disposable home; an independent accepted-outcome parent-recruiter pin applies only to the exact recruiter route and its repair, fails closed on config/environment mismatch, and preserves planner, ordinary-turn, and other-canary routes; inference profiles materialize only into the child canary's provider tuple and never enter the ordinary chain; requested and actual answering providers are recorded separately without credentials; nonce-bound proof; attestations bind to host, OS, version, capability, installation, and current launcher identity. An exact-config canary may project only bounded present credential values whose credential-shaped ASCII names occur in that validated config, do not collide with the built native/control environment, and do not use the canary-control prefix, and only after the normal minimal CLI environment is built; the tool-reduced Codex/Claude invocation retains those values in process memory only, while malformed names or values fail before launch and the general CLI allowlist remains unchanged. Attended current-profile Codex verification first uses a strongly contained, bounded read-only app-server exchange that selects only `agency-preflight@agency-runtime`, requires the canonical events exactly once with valid current hashes, and requires each to be enabled and `trusted`; warnings, errors, missing or duplicate events, unknown shapes or statuses, timeouts, disabled, untrusted, or modified entries fail closed before a model call. Explicit autonomous verification instead requires the exact owned Codex invocation to carry its supported `--dangerously-bypass-hook-trust` option and records `trust_mode=autonomous_bypass` plus a `bypassed` receipt, never `trusted`; it neither reads around nor modifies persistent trust state. Both modes require the same nonce-bound behavioral proof. A live staffing canary binds a fresh installed host session to the exact inference decision and card hashes, then observes whether the native harness independently creates a child. Success requires the native host's own artifact to contain every selected card hash before that child's first speech, with exact parent-child, trace, host version, plugin, package, and candidate identities. Multiple compatible cards must reach at least one child. Agency Store rows, model prose, simulation, registration, trust state, and an older artifact cannot rescue a missing or malformed native artifact. An opaque host channel remains `unproven` or `negative` until the host-backed channel is engineered; it is not an evidence exception. Only sanitized counts, hashes, event names, statuses, current identities, and bounded trust-mode facts survive; command strings, card content, source paths, unrelated hooks, host ciphertext, and raw process output do not. Product trials retain the workspace-write sandbox with no added directory, bind any sentinel to the exact evaluated invocation, and exclude persistent profile configuration; preexisting, missing, mismatched, linked, oversized, or uncleanable proof fails closed. |
...
| Durable dashboard access (opt-in, AR-436 / ADR-0248) | Off by default. When enabled, the service reuses one owner-private token from `~/.agency-runtime/run/dashboard-access.json` (same owner-only, identity-checked directory and publication path as the rotating descriptor) and the page keeps it in the browser profile's local storage for the loopback origin. This lengthens the token's lifetime; it does not widen who can reach it: reading it still requires the owner's home directory or that browser profile. A 401 clears the remembered token; uninstall rotates it. |
...
Earlier revisions treated plan-row dispatch, work units, planned-child
activation, single-use activation capabilities, and a separately dispatched
finalization job as the delivery transport. Their implementation reasoning
remains in Git history and in [ADR-0054](decisions/0054-unit-aware-assignment-and-event-driven-dag.md),
[ADR-0070](decisions/0070-run-child-specific-agency-activation.md),
[ADR-0128](decisions/0128-persist-exact-codex-plan-authority-and-serialize-launches.md),
and [ADR-0136](decisions/0136-bind-opaque-codex-execution-by-ciphertext-identity.md).
Those records are provenance, not current AR-119 acceptance or threat-control
authority. The founding vision now requires inference-owned card choice,
host-owned spawning, in-context loading, and host-authored delivery proof; the
retired transports cannot satisfy those properties.
...
- Host-authored child artifacts establish the evidence authority boundary, not
  a cryptographic trust boundary against the native host. A compromised host or
  same-account plugin can forge its own transcript, and remains inside the
  stated local-user boundary. Exact card hashes before first child speech and
  native parent-child correlation prevent an Agency-written Store row, later
  transcript mention, or model claim from being promoted into Rule-4 proof;
  they do not make a hostile host trustworthy.
  Codex's current Sol child context and all 65 observed Desktop calls are
  encrypted, while Codex also exposes a conditional marked plaintext path.
  ADR-0159's separate exact CLI and Desktop-profile attestations plus adversarial
  simulation guard that source path, but they are not installed or live host
  proof; Codex therefore remains unproven in the evidence matrix until one exact-
  candidate host artifact satisfies ADR-0156. Temporal
  correlation, task labels, plaintext appearance, Store lifecycle rows, or
  generic MCP attribution cannot fill that gap. Each supported host still needs
  its own current installed and live artifact, and no host's evidence
  substitutes for another's.
...
- A valid inference result is authoritative for which cards Agency selected,
  but it is not proof that the work was good, that a native child received the
  cards, or that a requested provider/model actually served the call. Host
  artifacts and independent outcome verification establish those separate
  claims; response telemetry alone reconciles actual provider/model identity.
  A LiteLLM router or requested alias remains a separate field.
- Distinct producer, verifier, and outcome identities reduce self-review and
  duplicate-credit risk in contractor promotion, but they do not prove the
  verifier is correct or free from shared provider failure. The seven-day
  review window and three accepted outcomes are minimum admission controls, not
  a quality guarantee; later evidence may still revoke or quarantine the card.
...
- OpenClaw's preflight tells the model to construct its first visible response
  through the local `agency.finalize` tool. Its finalize hook never requests a
  model revision; a first invalid natural response is remembered as terminal.
  `reply_payload_sending` requires a synchronous payload-bound grant and
  `message_sending` consumes its one-use dispatch seal.
  That seal prevents terminal-response replay; it is not specialist activation,
  a staffing decision, or Rule-4 delivery evidence.
