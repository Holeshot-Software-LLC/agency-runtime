...
- Host-authored child artifacts establish the evidence authority boundary, not
  ...
  Codex's opaque child context is a current engineering gap: where the host
  does not expose an integrity-bound card-delivery channel, the result remains
  negative or unproven in the evidence matrix. Temporal correlation, task
  labels, Store lifecycle rows, or generic MCP attribution cannot fill that
  gap. Each supported host needs its own current installed and live artifact,
