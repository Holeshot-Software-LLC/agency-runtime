...
| Native lifecycle authority | Bare install auto-discovers installed supported harnesses and uses each harness's native registration, enablement, and trust lifecycle. Agency ships no independent Windows Hello verifier and exports no reusable authorization result. Exact install namespaces are closed-world parsed and serialize host work on the owner-private lifecycle lock. Roster rollback, owned host uninstall, and generic positive controls remain unavailable. MCP, hooks, broker, and dashboard request paths remain read-only for persistent mutations. Codex activation verification is evidence-producing rather than installation-authoritative and cannot change configuration, controls, roster/workforce, dashboard, adapter, registration, or trust state. |
