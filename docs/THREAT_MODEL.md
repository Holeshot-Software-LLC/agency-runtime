...
| Codex child rewrite authorization | A Codex collaboration message is rewritable only when a version-pinned, bounded scan of canonical private host transcripts binds the exact session, turn, call, namespace, full arguments, and explicit empty encrypted-argument marker on the current authorization call. Exact CLI `0.147.0` in-file ancestry and authentic one-record TUI forks are supported: cross-file forks require unique canonical parent/root rollouts, bounded UTC-day plus-or-minus-one lookup with independently validated offsets, exact causal start joins at every edge, and sealed prefixes. Ancestor causal calls accept only the observed ordinary schema or that exact schema plus an empty marker; the current call still mandates the marker. Canonical padded paths, non-nil observed UUID domains, lexical and descriptor identities, private permissions, link and reparse rejection, exact record and prefix digests, repeated reattestation, a 64 MiB aggregate external-ancestry limit, and an atomic one-use Store guard reject substitution, TOCTOU, concurrency, and replay. Missing, null, nonempty, ambiguous, completed, oversized, unsupported-version, unsupported-ancestry, or schema-drift evidence fails open unstaffed. Desktop alpha and exec depth-two/deeper remain unsupported. This authority permits only the rewrite; it cannot replace the host-authored child artifact required to prove delivery. |
...
- Host-authored child artifacts establish the evidence authority boundary, not
  ...
  Codex's current Sol child context is encrypted, while Codex 0.147 also exposes
  a conditional marked plaintext path. ADR-0159's exact-call attestation and
  adversarial simulation now guard that source path, but they are not installed
  or live host proof; Codex therefore remains unproven in the evidence matrix
  until one exact-candidate host artifact satisfies ADR-0156. Temporal
