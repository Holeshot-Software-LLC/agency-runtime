...
| Host-scoped soft control | Canonical SQLite row per host; absent state is enabled at generation zero; status projects the committed generation; the dormant mutation contract compares one observed generation under `BEGIN IMMEDIATE`, increments real transitions exactly once, and preserves no-op revisions. Every public CLI mutation first reaches a shared parsed-namespace presence guard; the production OS verifier is unavailable, so it fails before dispatch. No positive path yet prepares and binds authoritative resource state and deferred input across native verification and the committing boundary, presents the resolved transition for informed approval, or avoids a stable digest oracle for low-entropy secrets; AR-143 remains the explicit release blocker. Dashboard, MCP, generated-host, and restricted-Windows brokerage remain read-only; native lifecycle is never proxied. |
...
- The owner dashboard bearer is not proof of human presence when a model can
  ...
  AR-143 keeps production blocked until a positive CLI path is gated by a real
  OS-backed, non-exporting presence result and revalidates its prepared
  authoritative mutation inside the committing transaction. Any future
  transferable capability also needs expiry and atomic replay protection.
