...
| Specialist activation capabilities | Single-use, exact-version and work-unit scoped. Public evidence is digest-only; transient private Codex authority may retain exact bounded repository-relative paths until the parent turn terminalizes. Retrieval and a terminal readiness turn remain distinct from proof that a named specialist executed. Codex execution requires one separately claimed goal-hash-bound follow-up, byte-equal parent/child ciphertext, and its causal terminal child turn. |
...
| Selection and prompt composition | Turn intent, expertise selection, and execution topology are separate decisions. Hard host, platform, tool, permission, activation, and policy filters run before inference. Configured inference is mandatory for selection-requiring turns. Recruiter inference first defines the ideal owner from an open-ended specialist pool, then accepts an installed roster worker only when it faithfully fits; the roster is a cache, not the expertise boundary. A valid zero-candidate gap can trigger inference-designed, narrowly scoped contractor hiring and same-turn restaffing. Deterministic code may recall candidates and reject unsafe proposals but cannot select, broaden, or amend a near-match into the task owner. Inference receives bounded structured cards, never full prompt bodies. Compatible-set construction enforces `requires`, `conflicts_with`, authority, context mode, independence, and resource constraints before hydration. One directive specialist per worker is the default; implementers and independent reviewers remain isolated. The resident `agency-steward` is parent-only infrastructure and cannot become a domain worker. If no accepted specialist or contractor receipt exists, the turn fails open (ADR-0152): no specialist is selected, the exact cause is reported and persisted, and the host answers as a generalist with a `Recruited via: none` header; the hard block remains only for non-staffing integrity failures that Agency can actually establish. On the native hosts, Agency's own unavailability is no longer one of them: when the verifier cannot run, the evidence contract cannot be read, or a terminal outcome cannot be persisted, the turn publishes and the failure is recorded at error level with the turn closed `verification_failed`, because a check Agency could not perform is not a finding about the response. A definite negative still terminalizes -- a response the verifier evaluated and rejected, or one that does not match the digest accepted for its trace -- as do an unreadable `Stop` envelope, whose owner Agency cannot even establish, and `UserPromptSubmit` preflight. OpenClaw and Hermes deliberately retain their stricter documented behavior described below, which is a known and currently unresolved host asymmetry rather than an oversight. |
...
| Evidence and finalization | Fresh printable trace and session IDs bounded to 512 UTF-8 bytes before lookup or indexed persistence; privacy-safe request fingerprints and durable typed turn classifications; explicit work-unit identity; exact Agency and host-native tool allowlists rather than namespace-suffix trust; duplicate and missing results fail; bounded work-unit extraction. Selection is a plan, not load evidence. Each isolated unit the native host actually starts requires a digest-only single-use capability bound to its ready-recipe slug, version, hash, and work unit; completion separately correlates retrieval with a generic native worker or tool-run receipt and never treats unauthenticated MCP access as named-specialist execution. For Codex, the initial child completion proves readiness only: the Store atomically claims one exact follow-up tool-use ID, and worker success requires its byte-equal parent/child ciphertext, exact lineage and task path, and later causal turn; the content-free execution identity remains bound to the verified activation goal hash. Declined, skipped, or response-invalid units close with explicit nonexecution evidence rather than a fabricated activation; historical `retry_exhausted` rows remain readable but are not created by current header enforcement; a host-merged unit is recorded as skipped with a bounded merge reason. Disabling an optional agent invalidates replay, preparation, consumption, and affected ready-turn completion; oversized exact prompts fail before selection or activation; mutation and terminal close transactions are mutually exclusive; terminal outcomes are monotonic and cannot be reopened by Stop feedback. Exact Store-backed headers are supplied or constructed before first publication; the first missing, malformed, stale, or mismatched natural response terminalizes without a continuation receipt or another model pass. Protocol input and output remain strict finite JSON; success is recorded only after verified outcomes; LiteLLM provenance is granted only by the callback-owned Store ingress; final headers reconcile against canonical SQLite evidence and reject spoofed or ambiguous fallbacks. |
| Installation and canaries | Canonical bundle digest and exact managed-tree comparison; unexpected files force replacement; owner-private staging; backups and rollback; native inventory postconditions; interpreter plus package-bootstrap lexical/resolved metadata/content identities persisted in the managed install manifest; launcher drift or unproven identity makes maturity stale and blocks registration; Windows permission setup rejects restricted or indeterminate tokens before DACL mutation; scheduled-task definitions use BOM-bearing UTF-16 input, token-SID-bound identity, bounded Base64/UTF-8 COM inspection, strict schema normalization, semantic execution checks, and exact pre-mutation requery; isolated canary homes receive an owner-only directory policy before credentials exist; bounded link-resistant authentication copy hardens the empty destination before writing; nonce-bound proof; attestations bind to host, OS, version, capability, installation, and current launcher identity. Attended current-profile Codex verification first uses a strongly contained, bounded read-only app-server exchange that selects only `agency-preflight@agency-runtime`, requires the canonical eight events exactly once with valid current hashes, and requires each to be enabled and `trusted`; warnings, errors, missing or duplicate events, unknown shapes or statuses, timeouts, disabled, untrusted, or modified entries fail closed before a model call. Explicit autonomous verification instead requires the exact owned Codex invocation to carry its supported `--dangerously-bypass-hook-trust` option and records `trust_mode=autonomous_bypass` plus a `bypassed` receipt, never `trusted`; it neither reads around nor modifies persistent trust state. Both modes require the same nonce-bound behavioral proof. Only sanitized counts, event names, statuses, current hashes, and bounded trust-mode facts survive; command strings, source paths, unrelated hooks, and raw process output do not. The exact nonce-bound parent task and canonical direct child goal are distinct. Plaintext launches retain exact goal equality. An opaque Codex launch instead requires one exact persisted native task label, preserves the host ciphertext, and injects a token-free v2 child context carrying that row's goal hash because neither relevant hook exposes the decrypted goal. Preflight atomically stages its exact private path scope; only a repository-wide sentinel becomes `.`, and a second opaque grant is denied until `SubagentStart` consumes the first. The spawned child completes an activation-only turn, then one exact `followup_task` delivery is atomically claimed; the parent call and child's later `NEW_TASK` record must contain byte-equal ciphertext under exact session, tool-use, lineage, task-path, and turn evidence. No retry or alternate message path is accepted, and ciphertext never enters reports or Store. A proven owned-process timeout projects only a fixed allowlisted reason, never raw output. Current-profile activation verification uses exactly one canary; success requires the fresh invocation's hook start, inference receipt, route, specialist injection, both causal native child turns, exact execution dispatch, finalization, trace, scope, and installation identity to correlate, so an older attestation cannot rescue a failed or malformed attempt. Codex product trials write one canonical trusted-project entry only inside the disposable profile, retain the workspace-write sandbox with no added directory, correlate the exact executed prompt to the activation snapshot, and require a bounded prompt-derived sentinel written by that invocation before grading. Persistent profile configuration is excluded; preexisting, missing, mismatched, linked, oversized, or uncleanable proof fails closed. |
...
- Specialist activation tokens are scoped bearer capabilities, not
  ...
  command-hook delivery. Plaintext launches require the exact persisted goal.
  Opaque Codex launches require the exact current flattened name, a strictly
  shaped host ciphertext, one exact persisted task label, and a v2 child
  context carrying that row's content-free goal hash. Agency then atomically
  claims the sole unconsumed child start created after that grant.
