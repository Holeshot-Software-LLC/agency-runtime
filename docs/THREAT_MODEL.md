...
| Specialist activation capabilities | Single-use, exact-version and work-unit scoped. Public evidence is digest-only; transient private Codex authority may retain exact bounded repository-relative paths until the parent turn terminalizes. Retrieval remains distinct from proof that a named specialist executed. |
