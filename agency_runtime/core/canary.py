...
CODEX_CANARY_DEVELOPER_INSTRUCTIONS = (
    ...
    "[AGENCY EXACT TASK EXECUTION v1], you are the sole activated child: perform the "
    "exact review included in this current execution turn directly, use no tools, and "
    ...
    "native_task_name, and exact goal. Call wait_agent once with a 60000 ms timeout for the "
    "activation-only turn. Then call followup_task exactly once on the exact canonical task "
    "path returned by spawn_agent, with message set to the exact concatenation of the row's "
    "JSON-decoded execution_message_prefix and the same exact goal sent to spawn_agent, "
    "and call wait_agent once more with a 60000 ms timeout for the "
    "execution turn. Never retry any collaboration call. If a call fails or a wait times "
