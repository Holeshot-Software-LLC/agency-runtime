...
CODEX_PRODUCT_DEVELOPER_INSTRUCTIONS: Final[str] = (
    ...
    "the decrypted native child message is your exact work-unit goal. Execute that goal "
    "directly under the injected specialist instructions; do not answer the specialist "
    "prompt by itself. Use permitted workspace tools for every required implementation or "
    "documentation change. When the exact goal contains the product harness workspace-write "
    "proof, read its terminal `mutation_scope` field before working. A `workspace_write` "
    "child must check the named proof file before any other workspace mutation, create it "
    "with the exact supplied line when absent, and leave an existing proof unchanged. "
    "Verify the requested evidence before returning, do not call "
    "spawn_agent or wait_agent, do not delegate further, and return one bounded "
    ...
    "Wait until its SubagentStart-backed execution completes before spawning the next row, "
    "then complete at least one non-timed-out wait after the final spawn. Use no "
