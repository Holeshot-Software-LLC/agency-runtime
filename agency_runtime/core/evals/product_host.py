...
CODEX_PRODUCT_DEVELOPER_INSTRUCTIONS: Final[str] = (
    ...
    "documentation change, verify the requested evidence before returning, do not call "
...
def _prompt_with_workspace_write_proof(prompt: str, prompt_hash: str) -> tuple[str, str]:
    ...
    wrapped = (
        ...
        "The first delegated work unit with `mutation_scope=workspace_write` must create "
        "the relative file below as its first workspace mutation. Read-only children and "
        "the non-working parent must not create it. Later workspace-write children must "
        "leave it unchanged. File: "
