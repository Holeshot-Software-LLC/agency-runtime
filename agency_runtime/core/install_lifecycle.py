...
def is_exact_install_lifecycle(namespace: object) -> bool:
    ...
    production_container = getattr(namespace, "production_container", None)
    ...
    if (
        ...
        or (config is not None and (not isinstance(config, str) or not config or "\x00" in config))
        ...
        or type(production_container) is not bool
        or type(json_mode) is not bool
        or sum((dry_run, rollback, verify_activation)) > 1
        or (autonomous and not verify_activation)
        or (
            production_container and (rollback or verify_activation or autonomous or config is None)
        )
    ...
    if verify_activation:
        return bool(
            ...
            and profile is None
            and config is None
            and not production_container
            and backup is None
            and not no_dashboard
        )
    if rollback:
        return bool(
            agent is not None and not all_hosts and config is None and not production_container
        )
