...
(function installAgencyCharts(root, factory) {
	...
	function renderActivityChart(root, summaryRoot, activity, options = {}) {
		...
		const summary = `${routes} observed routes · ${delegations} recorded child events · last ${minutes} minutes`;
		...
		const svg = accessibleSvg(
			documentRef,
			"Observed routing and recorded native-child activity",
		...
		delegationPoints.forEach((point, index) => {
			...
			const circle = svgNode(documentRef, "circle", {
				...
				tabindex: 0,
				role: "img",
				"aria-label": `${buckets[index].delegations} recorded child events in this minute`,
			});
			circle.append(svgNode(documentRef, "title", {}, `${buckets[index].delegations} recorded child events`));
			svg.append(circle);
		});
	...
	function renderOutcomeChart(root, summaryRoot, activity) {
		...
		const summary = `${counts.total} recorded native-child events · ${counts.success} completed · ${counts.failed} failed · ${counts.skipped} skipped · ${counts.unknown} unknown`;
		if (summaryRoot) summaryRoot.textContent = summary;
		...
		const svg = accessibleSvg(
			documentRef,
			"Recorded native-child outcomes",
			`${summary}. Outcomes describe bounded stored host-native child evidence.`,
			"0 0 240 240",
			summaryRoot?.id || "",
		);
		const center = 120;
